...
const DashboardReport: React.FC = () => {
  ...
  const onDownload = () => {
    ...
    md += `> [!IMPORTANT]\n`;
    md += `> Provide this file to an AI assistant (Cursor, ChatGPT, or Claude) and say: \n`;
  ...
  const onShare = async () => {
    ...
    try {
      // Create a shareable link by encoding the report data
      const json = JSON.stringify(reportData);
      // Use a more robust base64 encoding that handles unicode
      const b64 = btoa(encodeURIComponent(json).replace(/%([0-9A-F]{2})/g, (match, p1) =>
        String.fromCharCode(parseInt(p1, 16))
      ));

      const url = new URL(window.location.href);
      url.searchParams.set('report', b64);
      ...
      alert('Could not generate share link. The report might be too large.');
    }
  };
          ...
          <div id="resSec">
                ...
                <div style={{ pointerEvents: 'auto', flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12, position: 'relative' }}>
                  ...
                  {showActionMenu && (
                    <div className="cs-action-popover" onClick={(e) => e.stopPropagation()}>
                      ...
                      <button className="cs-menu-item" onClick={() => { onShare(); setShowActionMenu(false); }}>
            ...
            <div className="cs-page" style={{ paddingTop: 40 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 28 }}>
                ...
                {usage.total > 0 && (
                  <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', padding: '6px 12px', borderRadius: 10, display: 'flex', gap: 16, fontSize: 11, fontFamily: 'monospace', color: '#64748B' }}>
                    <span>IN: {usage.input.toLocaleString()}</span>
                    <span>OUT: {usage.output.toLocaleString()}</span>
                    <span style={{ color: '#0F172A', fontWeight: 'bold' }}>Σ {usage.total.toLocaleString()}</span>
                  </div>
                )}
              </div>
